...
function Pictures({ path, alt, type, order }: { path: string, alt: string, type: Type, order: number }) {
    ...
    if (order === 1) {
        ...
        frame = (
            <div className="rotate-[-5deg] translate-x-[0px] translate-y-[-100px] absolute inset-0 flex justify-center items-center p-2 pb-[40px] bg-[#BDC7D0]">
                {picture}
            </div>
        );
    } else if (order === 4) {
        ...
        frame = (
            <div className="rotate-[0deg] translate-x-[0px] translate-y-[-100px] absolute inset-0 flex justify-center items-center p-2 pb-[40px] bg-[#BDC7D0]">
                {picture}
            </div>
        );
    }
...
export default function CelebratePast() {
    const [images, setImages] = useState([
        ...
        { path: "/realclouds.jpg", alt: "among us!!!", type: Type.Square, order: 4 },
    ]);
        ...
        const interval = setInterval(() => {
            setImages(prevImages => {
                return prevImages.map(img => ({
                    ...img,
                    order: img.order === 1 ? prevImages.length : img.order - 1,
                }));
            });
        }, 15000); //15sec
